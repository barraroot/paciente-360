...
- **Active feature**: `018-ai-multimodal-mcp` — **DELIVERED 2026-05-31** — Conversa Reativa, Multimodal e Auto-Curadoria do Kanban via IA. Cobriu 4 gaps da Fase 17: coalescência híbrida (US1), lead onboarding automático (US2), auto-curadoria do kanban (US3), STT/TTS (US4/US5), Persona test chat sandbox (US6), servidor MCP local com circuit breaker + fallback runtime (US7). Polish: rate limit anti-abuso 2 camadas + cooldown auditável, consent `Transcricao` LGPD-aware, runbook MCP cut-over + Grafana + Prometheus alerts. **Constitution 7/7 PASS** com 1 desvio (latência fim-a-fim p95 ≤12s target, sem amendment) + 1 desvio D2 (E2E DEFERRED, padrão das fases anteriores). Detalhe em "Fase 18 — Key Patterns" abaixo.
...
## Conversa Reativa + Multimodal + MCP (Fase 18) — Key Patterns

When working on coalescência/MCP/multimodal/kanban auto-curadoria post-Fase 18, remember:

1. **Coalescência híbrida (US1) = passive debounce + cancel-and-reprocess via versão Redis** — `ConversationTurnCoordinator` mantém `ai:turn:{conv}:v` (INCR atômico), `ai:turn:{conv}:msgs` (LIST), `ai:turn:{conv}:debounce` (SET NX EX). `PassiveDebounceScheduler` agenda `FlushCoalescedTurnJob` com delay `ai.matricial.coalesce.passive_debounce_s` (default 4s). `TurnVersionGuard::assertCurrent()` no `ProcessAiResponseJob` pré E pós processamento; se versão mudou e `< max_reprocesses` (default 3) → re-agenda; senão → dispatcha com o que tem (FR-004). **Ordem das mensagens no turno é cronológica de chegada** (FR-007).

2. **MCP é Q2=B (substituição com fallback runtime)** — `App\Domain\Ai\Mcp\Server\PacienteMcpServer` expõe 7 capabilities (`get-clinic-info`, `list-professionals`, `get-availability`, `get-current-patient`, `create-or-find-lead`, `hold-slot`, `update-lead-profile`). `ToolRunner::run()` é o ponto único de seleção: se `AI_TOOLS_VIA_MCP=true` E `McpCircuitBreaker::shouldAllowMcpCall()` → `McpToolBridge` (HTTP local); senão → tool nativa (Fase 17 preservada FR-052). Em falha do bridge: CB registra failure + `ToolRunner` cai para nativa runtime — atendimento NÃO para (FR-053b). **Cut-over operacional**: `docs/runbooks/fase18-mcp-cutover.md` (5 gates obrigatórios — não promover flag sem todos verdes).

3. **Circuit breaker Redis-backed com estados closed/half_open/open + auto-revert** (`McpCircuitBreaker`):
   - `recordFailure(errorCode, message)` em closed: INCR `mcp:cb:failures` (TTL `failure_window_s`); 3 falhas (default) → `transitionToOpen` com `initial_cooldown_s` (default 60s) e dispatcha `McpCircuitOpened(source='automatic')`.
   - `state()` é lazy — em `open`, se `cooldown_s` passou → transita para `half_open` (canário).
   - `shouldAllowMcpCall()` em `half_open` pega `mcp:cb:canary_in_flight` via `Redis::set(NX EX 30)` — só UMA canário por vez.
   - `recordSuccess()` em `half_open` → `closed` + reset failures.
   - `recordFailure()` em `half_open` → reabre com `cooldown * 2` (cap `max_cooldown_s` default 600).
   - `recordManualRollback($actorUserId)` (admin desligou flag) → emite `McpCircuitOpened(source='manual_flag')` para audit distinguível.
   - Snapshots persistidos em `mcp_circuit_breaker_snapshots` via `PersistMcpCircuitSnapshotListener` (handlers subscribe via `subscribe()` no `EventServiceProvider`).

4. **Sandbox MCP via `mcp.sandbox` ability** — `McpTokenGuard` detecta `$token->can('mcp.sandbox')` e chama `SandboxContext::enable($token->id)`. Em capabilities de escrita (`CreateOrFindLead`, `HoldSlot`, `UpdateLeadProfile`), o `BaseMcpCapability::handle()` desvia para `runSandbox()` (output sintético — NÃO persiste em `pacientes`/`slot_reservations`). Capabilities de leitura passam direto (sandbox é fiel sobre dados reais — FR-041). Guarda estrutural em `IaCannotAutoConfirmTest` impede alguém de inserir capability nova que mova card para "agendado"/"confirmado" — esses status só transitam via listeners de domínio (`PromoteToScheduledOnHoldPlaced`/`PromoteToConfirmedOnReservationPaid`).

5. **Lead onboarding automático (US2) é ANTES da coalescência** — `EnqueueLeadOnInboundMessageListener` (`Listeners\Crm\Kanban\`) escuta `MensagemRecebida` e chama `LeadOnboardingService::ensureFor(channelType, identifier, tenantId)`. Lookup por `telefone_primario_normalizado` (WA) ou `instagram_handle` (IG) ou opaque (widget). **Paciente regular (status!='lead') NÃO entra no kanban** (Q-clarify-3=B / FR-011a) — conversa anexa ao prontuário; audit `KanbanCurationEvent(event_kind='inbound_attached_to_existing_patient')`. Lead em coluna terminal → reabre para `is_initial=true` com `event_kind='lead_reactivated'`.

6. **Auto-curadoria do kanban (US3) — `UpdateLeadProfileCapability` + listeners de transição** — Allow-list de campos NÃO clínicos (`name|complaint|preferred_city|urgency|procedure|price_range`); PII clínica rejeitada pelo `PiiScrubber::detectClinical($value)` → `error_code: clinical_pii_blocked`. **FR-020 não-regressão sob manual override**: `KanbanAutoTransitionService` verifica se `Paciente.funil_coluna_changed_by_user_at` é mais recente que o evento — se for, supressão com `KanbanCurationEvent(applied=false, suppression_reason='manual_override')`. Listeners: `PromoteToScheduledOnHoldPlaced` (SlotReservation::created holder_type='ia'), `PromoteToConfirmedOnReservationPaid` (AppointmentConfirmed), `PromoteToHumanOnEscalation` (AiAssignmentEscalatedToHuman), `DowngradeToLostOnInactivityListener` (cron daily 03:00).

7. **STT inbound (US4) — Whisper, fila `transcription`, fallback gracioso** — `TranscribeInboundAudioJob` enfileirado por `ProcessInboundMessageJob` quando `mídia=áudio AND canal!=widget`. `AudioTranscriptionService::transcribeInbound()` persiste em `audio_transcriptions`, atualiza `messaging_messages.content` com texto + `is_audio_origin=true`, passa por `PiiScrubber` ANTES de gravar (FR-055b). Falha (silence/lang/timeout/corruption) → insere `Message system="áudio não entendido"` na conversa, IA NÃO é invocada (FR-027). **Widget de site ignora STT** (decisão FR-026).

8. **TTS outbound (US5) — só sob gatilho explícito** — `AudioPreferenceDetector::prefersAudioForTurn(Turn)` lê `config('messaging.audio.outbound.triggers')` (lista de 12 frases PT-BR normalizadas — "nao sei ler", "to dirigindo" etc) e faz match por substring. **Voz** vem da Persona via `PersonaVoiceResolverService` (cadeia: `Persona.voice_id` → `tenant.default_voice_id` → entrada `is_system_default=true` — Q-clarify-4=B). `AudioSynthesisService` orquestra `TtsTextNormalizer` (R$, horários, abreviações → fala) + `ElevenLabsProvider`. Em falha: `fallback_to_text=true` → texto enviado normalmente, audit emite `ai_tts_fallback_to_text_total`. **Master switch tenant `tts_enabled`** desliga TTS inteiro. **Widget ignora TTS** (FR-032). `VoiceCatalogResource.toArray()` NUNCA expõe `provider_voice_id` ao tenant (FR-037c — só no super-admin Filament).

9. **Persona Test Chat sandbox (US6)** — `PersonaTestSessionController::store` emite Sanctum PAT com abilities `['mcp.invoke', 'mcp.sandbox']` + `tenant_id` na coluna nova em `personal_access_tokens`. Echo channel `private-persona-test.{session_id}` (`PersonaTestMessageBroadcasted` broadcast event). `SandboxOutboundDispatcher` registra `Message` com `sandbox=true, sandbox_session_id=...`. **Métricas isoladas (FR-042)**: `DashboardHomeService`, `ExecutiveDashboardCollectors`, `AiUsageMetricsCollector`, `MessagingMetricsCollector` filtram `WHERE sandbox=false`/`WHERE sandbox_session_id IS NULL`. Close → revoga PAT imediato (`PersonalAccessToken::delete()`); abrir nova sessão (admin, persona) auto-fecha anterior com motivo `superseded`. UI: botão "Testar" em `PersonasIndex.vue` + "Testar persona em edição" em `PersonaForm.vue` (envia `use_draft=true, persona_draft=formData` FR-039).

10. **Rate limit anti-abuso (Polish T200–T207) em 2 camadas + cooldown auditável** — `InboundConversationLimiter::checkOrThrow(convId, tenantId, identifier)` usa `RateLimiter::hit/tooManyAttempts` (registrados em `RouteServiceProvider`):
    - `per_conversation`: 30 msgs/10min (default).
    - `per_identifier`: 100 msgs/10min do mesmo telefone/handle.
    - Per-conv checa primeiro; se OK, hits per-identifier. Excedido → `RateLimitExceededException` capturada pelo listener → `CooldownService::startFor()` (popula `cooldown_until`/`cooldown_reason`, eleva `priority='alta'`, posta `Message system`, emite `ConversationCooldownStarted` auditável).
    - **Guards em 2 camadas**: `TriggerAiResponseOnInboundMessage` (early bail) + `ProcessAiResponseJob.handle()` (chokepoint final) chamam `IsConversationOnCooldownChecker::check()` — lazy end emite `ConversationCooldownEnded(endedBy='expired')` quando janela passou (sem cron).
    - **Mensagens inbound continuam sendo PERSISTIDAS** durante cooldown — operador segue podendo responder manualmente.
    - **`BurstClassifier`** (Levenshtein + palavras-gatilho + CAPS ratio) classifica `spam`/`crisis`/`unknown` SÓ para enriquecer o `burst_label` do evento — não altera comportamento.
    - Permission `messaging.cooldown.manage` (admin-clinica + atendente). `POST /api/v1/inbox/conversations/{id}/cooldown/end` (idempotente, cross-tenant 404). Badge "🚫 cooldown Nmin" no `ConversationListItem.vue`.

11. **Consent `Transcricao` LGPD-aware (Polish T208–T213)** — Novo valor no enum `ConsentFinalidade::Transcricao` (`Migration ADD VALUE IF NOT EXISTS`, NÃO reversível em PostgreSQL). Texto da transcrição (`audio_transcriptions.transcribed_text`) permanece indefinidamente (sem voz biométrica). O ÁUDIO BRUTO em `messaging_message_media`:
    - **Cron diário 04:00 BRT** (`PurgeExpiredAudioRawJob`, fila `privacy`) varre `media.created_at < now - default_days` (config `messaging.audio.retention.default_days`, default 90). Se paciente tem `ConsentFinalidade::Transcricao` ativo → preserva. Senão → `Storage::delete()` + `storage_path=null` + `media_purged_at=now()`. Emite `AudioRawPurged(reason='expired_no_consent')` UMA vez por execução.
    - **Revoke retroativo** (`PurgePatientExtendedAudioJob`): listener `PurgePatientAudioOnConsentTranscricaoRevoked` enfileira por paciente assim que revoke é registrado (filtra `finalidade=Transcricao`).
    - **`storage_path` é nullable** (migration `make_storage_path_nullable_in_messaging_message_media`) — Fase 3 era NOT NULL.
    - 3 endpoints `/api/v1/pacientes/{p}/consents/transcricao/{show|grant|revoke}` (`TranscricaoConsentController`) com `audios_to_purge` count no response do revoke. UI `pacientes/ConsentTranscricaoToggle.vue` com modal "saiba mais" + confirmação a11y.

12. **Observabilidade (T214/T215)** — Dashboard Grafana `docs/observability/grafana/paciente360-fase18-overview.json` (12 painéis: CB state stat, MCP p95+erro, coalesce msgs-per-turn, STT/TTS p95+fallback, rate limit cooldowns, kanban curation, tool round-trips, msgs/escalações). Alertas Prometheus `docs/observability/prometheus/fase18-alerts.yml` (8 alarmes — 2 page, 6 warn). Runbook cut-over `docs/runbooks/fase18-mcp-cutover.md` é a fonte da verdade operacional.

13. **DEFERRED ao final da Fase 18** (desvio consciente D2, padrão das fases anteriores)
    - E2E Playwright `tests/e2e/ai-multimodal-conversation.spec.ts` (T218) e `persona-test-chat.spec.ts` (T219) — cobertura via Feature tests determinísticos.
    - SLA bench (`tests/Feature/Ai/Sla/Phase18BenchmarkSlaTest.php` `@group=sla-benchmark`): 4 cenários SCAFFOLDED (`markTestSkipped` com razão), implementação concreta no CI noturno pré-cut-over (runbook §1 G3).
    - **Cut-over MCP em si** (`AI_TOOLS_VIA_MCP=true`) é decisão operacional pós-DELIVERED, governada pelo runbook.
