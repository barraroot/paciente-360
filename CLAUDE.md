...
- **Active feature**: `015-ai-matricial` — [plan](specs/015-ai-matricial/plan.md) — IA Matricial: camada **plugável** de configuração+orquestração de IA sobre o omnichannel existente (não recria canais/conversas/auth/tenancy). Spec (8 US P1–P3, 43+ FRs, 13 SC, checklist 16/16) + 4 clarifications (Session 2026-05-25: bases via **RAG/embeddings**; **auto-envio com escalonamento**; credenciais do provedor **globais da plataforma**; gatilho = toda inbound com **debounce**) + plan + research (R1–R12) + data-model (11 tabelas `ai_*`, só `ai_models` é global; reusa `ai_paused_until`/eventos/`MessageDispatchService`/`PiiScrubber`) + contracts (gates G1–G12). **Constitution Check PASS 7/7 sem amendment** — Princípio III (Segurança Clínica IA, NN) já antecipa esta camada: guardrails mínimos em código + intenção + score de confiança + escalonamento + auto-pause + log auditável ≥6m. Round-robin por `(tenant,channel)` com `lockForUpdate`; listener em `MensagemRecebida` → `ProcessAiResponseJob` (fila `ai`); catálogo de modelos no Filament super-admin. **Deps APROVADAS 2026-05-26**: extensão `pgvector` (RAG) + pacote oficial **`laravel/ai`** (Laravel AI SDK v0.x — Agent/`HasStructuredOutput` p/ intenção+confiança, `Embeddings` p/ RAG, fakes p/ testes; provider Anthropic, credenciais globais via env). Persona = Agent dinâmico (`PersonaAgent`). Próximo: `/speckit-tasks`.
- **Previous features delivered (13)** — sumário enxuto; detalhe técnico de cada fase vive nos blocos "Key Patterns" abaixo e em `specs/<feature>/`:
