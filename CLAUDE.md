...
- **Active feature**: `016-frontend-ux-audit` — [plan](specs/016-frontend-ux-audit/plan.md) — Auditoria e correção de UI/UX do frontend (SPA Vue): campos/botões fora do layout, consistência desktop+responsivo, acessibilidade e i18n. Spec (5 US P1–P3) + plan + research (R1–R8) + data-model (catálogo de problemas, **sem persistência de app**) + contracts (G1–G16 invariantes verificáveis: overflow, preenchimento, alvos de toque, foco/contraste AA, chaves i18n cruas, sem diálogo nativo, `v-html` sanitizado). **Constitution Check PASS 9/9 sem amendment** — feature corretiva, só `resources/js/**` + `pt-BR.json`. Breakpoints alvo **375/768/1366/1880**; referência = "design de fato" das telas de melhor qualidade. Verificação via Playwright (geometria) + axe (a11y) + varredura i18n. Próximo: `/speckit-tasks`.
