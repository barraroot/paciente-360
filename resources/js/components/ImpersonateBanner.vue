...
<template>
    <div
        ...
        role="alert"
        aria-live="polite"
        class="sticky top-0 z-[100] w-full bg-warning-400 text-warning-950 shadow-md"
    >
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2 text-sm">
            <div class="flex items-center gap-2">
                ...
                <span
                    >— você está visualizando como suporte. Todas as telas visitadas são
                    auditadas.</span
                >
            </div>
            <div class="flex items-center gap-3">
                <span v-if="endError" class="text-xs font-medium text-danger-800">{{
                    endError
                }}</span>
                <button
                    type="button"
                    :disabled="ending"
                    class="rounded border border-warning-800 bg-warning-300 px-3 py-1 text-xs font-medium hover:bg-warning-200 disabled:opacity-50"
                    @click="endSession"
                >
