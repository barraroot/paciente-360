...
async function onSubmit() {
    ...
    if (form.value.events_subscribed.length === 0) errors.value.events_subscribed = 'Selecione ao menos um evento.'
        ...
        >
            <div class="modal-panel">
                <header class="modal-panel__header">
                    ...
                    <button type="button" class="btn-icon" aria-label="Fechar" @click="emit('close')">×</button>
                </header>
                ...
                <form class="modal-panel__body" @submit.prevent="onSubmit">
                    ...
                    <div class="field">
                        ...
                        <input id="wh-url" v-model="form.url" type="url" placeholder="https://api.exemplo.com/webhook" />
                    ...
                    <fieldset class="field">
                        ...
                        <div class="events-grid">
                            <label v-for="ev in allEvents" :key="ev" class="event-checkbox">
                                <input type="checkbox" :value="ev" v-model="form.events_subscribed" />
                                <span>{{ ev }}</span>
                            </label>
                        </div>
                        <p v-if="errors.events_subscribed" class="field__error">{{ errors.events_subscribed }}</p>
                    </fieldset>
...
<style scoped>
.modal-overlay { position: fixed; inset: 0; background: rgba(15,23,42,0.5); display: flex; align-items: flex-end; justify-content: center; z-index: 50; }
@media (min-width: 640px) { .modal-overlay { align-items: center; } }
.modal-panel { background: white; border-radius: 0.75rem 0.75rem 0 0; padding: 1.25rem; max-width: 640px; width: 100%; max-height: 90vh; overflow: auto; }
@media (min-width: 640px) { .modal-panel { border-radius: 0.75rem; } }
.modal-panel__header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
.modal-panel__footer { display: flex; gap: 0.5rem; justify-content: flex-end; margin-top: 1rem; }
.field { margin-bottom: 1rem; }
.field label { display: block; font-weight: 500; margin-bottom: 0.25rem; }
.field input[type=text], .field input[type=url] { width: 100%; padding: 0.5rem; border: 1px solid #cbd5e1; border-radius: 0.375rem; }
.field__error { color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem; }
.events-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; }
.event-checkbox { font-size: 0.875rem; display: flex; gap: 0.5rem; align-items: center; }
