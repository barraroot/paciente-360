...
<template>
    <div
        ...
        class="bg-warning-100 border-b border-warning-300 px-4 py-2 flex justify-between items-center"
    >
        <span class="text-sm">
            ⚠️ Sincronização do Google Calendar interrompida. Suas consultas no CRM não estão sendo
            espelhadas.
        </span>
        <div class="flex gap-2">
            <button @click="reconnect" class="bg-primary-600 text-white text-sm px-3 py-1 rounded">
                Reconectar
            </button>
            <button @click="dismiss" class="text-foreground-muted text-sm">Dispensar</button>
        </div>
    </div>
</template>
