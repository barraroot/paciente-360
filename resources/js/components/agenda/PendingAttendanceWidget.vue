...
async function load() {
  const { data } = await api.get('/agenda/consultas', {
    params: { status: ['scheduled', 'confirmed'], from: new Date(Date.now() - 7 * 86400000).toISOString() },
  })
...
<template>
  <section class="border rounded p-4 space-y-2">
    <header class="flex justify-between items-center">
      ...
      <button @click="load" class="text-xs text-blue-600">refresh</button>
    </header>

    <p v-if="!items.length" class="text-sm text-gray-500">Nenhuma consulta pendente.</p>

    <ul v-else class="divide-y">
      <li v-for="apt in items" :key="apt.id" class="py-2 flex justify-between items-center">
        <span class="text-sm">
          {{ renderer.formatDateTime(apt.starts_at) }}
          <span class="text-gray-500">— Paciente #{{ apt.paciente_id }}</span>
        </span>
