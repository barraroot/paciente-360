...
<template>
  <div v-if="!r" class="p-6 text-sm text-gray-500">Carregando relatório…</div>
  ...
  <div v-else class="space-y-6 p-6">
    <header class="flex items-start justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-600">
          ...
          Disparada em: {{ fmtDate(r.dispatched_at) }}
        </p>
        <p v-if="r.status === 'dispatching'" class="mt-1 text-xs text-amber-700">
          ⚡ Em disparo — relatório atualiza automaticamente a cada 30 segundos.
        </p>
      </div>
      <RouterLink :to="`/campaigns/${r.campaign_id}`" class="text-sm text-gray-600 underline">Voltar</RouterLink>
    </header>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
      ...
      <div class="rounded border bg-white p-4">
        ...
        <p class="mt-1 text-2xl font-bold text-emerald-700">{{ r.total_dispatched }}</p>
      </div>
      <div class="rounded border bg-white p-4">
        <h3 class="text-xs font-medium text-gray-500">Bloqueados</h3>
        <p class="mt-1 text-2xl font-bold text-rose-700">{{ r.total_blocked }}</p>
      </div>
      <div class="rounded border bg-white p-4">
        <h3 class="text-xs font-medium text-gray-500">Agendamentos atribuíveis</h3>
        <p class="mt-1 text-2xl font-bold text-blue-700">{{ r.attributed_appointments }}</p>
      </div>
    </div>
    ...
    <section class="grid grid-cols-1 gap-4 md:grid-cols-2">
      <div class="rounded border bg-white p-4">
        ...
        <ul class="mt-3 space-y-1 text-sm">
          <li v-for="(count, status) in r.status_breakdown" :key="status" class="flex justify-between">
            <span>{{ status }}</span>
      ...
      <div class="rounded border bg-white p-4">
        ...
        <ul v-if="Object.keys(r.blocked_breakdown).length" class="mt-3 space-y-1 text-sm">
          <li v-for="(count, reason) in r.blocked_breakdown" :key="reason" class="flex justify-between">
            <span class="text-rose-700">{{ reason }}</span>
            <span class="font-mono">{{ count }}</span>
          </li>
        </ul>
        <p v-else class="mt-3 text-sm text-emerald-700">✓ Nenhum bloqueio.</p>
      </div>
    </section>
  </div>
</template>
