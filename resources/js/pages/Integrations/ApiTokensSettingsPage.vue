        ...
        <table v-else class="tokens-table">
            ...
            <tbody>
                <tr v-for="t in tokens" :key="t.id">
                    <td>{{ t.name }}</td>
                    <td><small>{{ (t.abilities ?? []).join(', ') }}</small></td>
            ...
            >
                <div class="modal-panel">
                    <header class="modal-panel__header">
                        ...
                        <button type="button" class="btn-icon" aria-label="Fechar" @click="modalOpen = false">×</button>
                    </header>
                    ...
                    <div v-if="lastToken" class="banner banner--success">
                        ...
                        <button type="button" class="btn btn--primary" @click="modalOpen = false; lastToken = null">Fechei</button>
                    </div>

                    <form v-else @submit.prevent="onSubmit">
                        <div class="field">
                            <label for="t-name">Nome (identifica o uso, ex.: "Zapier prod")</label>
                            <input id="t-name" v-model="form.name" type="text" maxlength="120" required />
                        </div>
                        <footer class="modal-panel__footer">
                            <button type="button" class="btn btn--secondary" @click="modalOpen = false">Cancelar</button>
...
.tokens-table th, .tokens-table td { text-align: left; padding: 0.5rem; border-bottom: 1px solid #f1f5f9; }
...
.modal-panel { background: white; border-radius: 0.75rem; padding: 1.25rem; max-width: 540px; width: 90%; }
.modal-panel__header { display: flex; justify-content: space-between; margin-bottom: 1rem; }
...
.banner { padding: 1rem; border-radius: 0.5rem; }
.banner--success { background: #d1fae5; color: #065f46; }
.banner code { display: block; padding: 0.75rem; background: white; border-radius: 0.25rem; word-break: break-all; margin: 0.5rem 0; font-size: 0.875rem; }
