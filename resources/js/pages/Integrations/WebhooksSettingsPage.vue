...
<template>
    <section class="page">
        <header class="page__header">
            <h1>Webhooks</h1>
            <button type="button" class="btn btn--primary" @click="openCreate">+ Novo webhook</button>
        </header>
        ...
        >
            ...
            <button type="button" class="btn btn--small" @click="store.clearSecret">Copiei, ocultar</button>
        ...
        <table v-else class="endpoints-table">
            <thead>
                <tr>
                    <th>Nome</th>
                    <th>URL</th>
            ...
            <tbody>
                <tr v-for="ep in store.endpoints" :key="ep.id">
                    ...
                    <td><small>{{ (ep.events_subscribed ?? []).length }} evento(s)</small></td>
                    <td>
                        <span class="badge" :class="ep.is_active ? 'badge--success' : 'badge--warning'">
                            {{ ep.is_active ? 'Ativo' : 'Pausado' }}
                        </span>
                    </td>
                    <td>{{ ep.failure_count }}</td>
                    <td class="actions">
                        <button type="button" class="btn btn--small" @click="openEdit(ep)">Editar</button>
...
.banner code { display: block; padding: 0.5rem; background: #fff; border-radius: 0.25rem; margin: 0.5rem 0; word-break: break-all; }
.endpoints-table { width: 100%; border-collapse: collapse; }
.endpoints-table th, .endpoints-table td { text-align: left; padding: 0.5rem; border-bottom: 1px solid #f1f5f9; }
