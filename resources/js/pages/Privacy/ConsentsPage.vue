...
<template>
  <div class="space-y-6 p-6">
    ...
    <section class="overflow-hidden rounded border bg-white">
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y">
          ...
          <tr v-else v-for="c in store.consents" :key="c.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <span class="rounded bg-gray-100 px-2 py-0.5 text-xs">{{ c.finalidade_label }}</span>
            </td>
            <td class="px-4 py-2">
              <span :class="['inline-block rounded border px-2 py-0.5 text-xs font-medium', stateClass(c.state)]">
                {{ c.state }}
              </span>
            </td>
            <td class="px-4 py-2 text-xs text-gray-700">{{ formatDate(c.granted_at) }}</td>
            <td class="px-4 py-2 text-xs text-gray-700">{{ formatDate(c.revoked_at) }}</td>
      ...
      <footer v-if="store.consentsPagination.total > 0" class="border-t bg-gray-50 px-4 py-2 text-xs text-gray-600">
        Página {{ store.consentsPagination.current_page }} de {{ store.consentsPagination.last_page }}
      ...
      >
        <div class="w-full max-w-md rounded bg-white p-6 shadow-xl">
          ...
          <div class="mt-4 space-y-3">
            ...
            <label class="block text-xs font-medium text-gray-700">
              Escopo
              <select v-model="revokeScope" class="mt-1 w-full rounded border-gray-300 text-sm">
