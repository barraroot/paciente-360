...
function urgencyClass(days) {
  if (days <= 2) return 'bg-rose-100 text-rose-900 border-rose-300'
  if (days <= 5) return 'bg-amber-100 text-amber-900 border-amber-300'
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y">
          <tr v-if="store.loading">
            <td colspan="6" class="px-4 py-12 text-center text-sm text-gray-500">Carregando…</td>
          </tr>
          <tr v-else-if="store.forgettingRequests.length === 0">
            <td colspan="6" class="px-4 py-12 text-center text-sm text-gray-500">
              Sem solicitações abertas. Solicitações vencidas/executadas ficam em "Histórico" (filtro futuro).
            </td>
          </tr>
          <tr v-else v-for="r in store.forgettingRequests" :key="r.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <span :class="['inline-block rounded border px-2 py-0.5 text-xs font-medium', urgencyClass(r.days_until_deadline)]">
                {{ r.days_until_deadline }} dia(s)
              </span>
            </td>
            <td class="px-4 py-2 text-right">
              <button type="button" class="mr-2 text-xs text-emerald-700 underline" @click="openExecute(r)">
                Executar
              </button>
              <button type="button" class="text-xs text-rose-700 underline" @click="openDeny(r)">
                Negar
              </button>
            </td>
          </tr>
    ...
    <Teleport to="body">
      ...
      >
        <div class="w-full max-w-lg rounded bg-white p-6 shadow-xl">
          <h2 id="exec-title" class="text-lg font-semibold">Confirmar execução do esquecimento</h2>
          <p class="mt-2 text-sm text-gray-700">
            Paciente <strong>#{{ target?.patient_id }}</strong> será anonimizado segundo o mapa Q26:
          </p>
          <ul class="mt-3 list-disc space-y-1 pl-5 text-xs text-gray-700">
            <li><strong>Anonimizar</strong>: nome, CPF, telefone, e-mail, data nascimento</li>
            <li><strong>Deletar</strong>: endereço, anotações livres</li>
            <li><strong>Preservar</strong>: receitas controladas (2a), billing (5a), audit (1a), consentimentos (5a)</li>
          </ul>
          ...
          <div class="mt-6 flex justify-end gap-2">
            <button type="button" class="rounded border px-3 py-1.5 text-sm hover:bg-gray-50" @click="showExecuteModal = false">
              Cancelar
            </button>
            <button
              type="button"
      ...
      >
        <div class="w-full max-w-lg rounded bg-white p-6 shadow-xl">
          ...
          <p class="mt-2 text-sm text-gray-700">Motivo (≥ 10 caracteres):</p>
          <textarea
            v-model="denyReason"
            rows="4"
          ...
          <div class="mt-4 flex justify-end gap-2">
            <button type="button" class="rounded border px-3 py-1.5 text-sm hover:bg-gray-50" @click="showDenyModal = false">
              Cancelar
            </button>
            <button
              type="button"
