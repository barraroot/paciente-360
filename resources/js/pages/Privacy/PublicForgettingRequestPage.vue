  ...
  <div class="mx-auto max-w-xl space-y-6 p-6">
    <header>
      ...
      <p class="mt-2 text-sm text-gray-600">
        Conforme o Art. 18º da Lei Geral de Proteção de Dados, você pode solicitar
        a exclusão dos seus dados pessoais. O processo respeita prazos legais de
        retenção (ex.: receitas controladas têm retenção mínima de 2 anos por força
        da Portaria SVS/MS 344/98).
      </p>
    </header>

    <div v-if="submitted" role="status" class="rounded border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
      <strong>Solicitação registrada.</strong> A clínica entrará em contato em até 15 dias úteis para confirmação
      da identidade e execução do esquecimento.
    </div>
    ...
    <form v-else @submit.prevent="submit" class="space-y-4 rounded border bg-white p-6">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700">E-mail de contato</label>
        <input
          v-model="form.email"
          type="email"
          required
      ...
      <div v-if="error" role="alert" class="rounded border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900">
        {{ error }}
      </div>
      ...
      <button
        type="submit"
        :disabled="submitting"
        class="w-full rounded bg-rose-700 px-4 py-2 text-sm font-medium text-white hover:bg-rose-800 disabled:opacity-50"
      >
        {{ submitting ? 'Enviando…' : 'Enviar solicitação' }}
      </button>

      <p class="mt-2 text-xs text-gray-500">
        Você receberá confirmação por e-mail. Para portabilidade de dados, use o formulário separado.
      </p>
    </form>
  </div>
</template>
