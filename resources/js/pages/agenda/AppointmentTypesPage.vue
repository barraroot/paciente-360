...
async function load() {
  ...
  try {
    ...
    error.value = 'Não foi possível carregar os tipos de atendimento. Verifique sua conexão e tente novamente.'
    types.value = []
  } finally {
    loading.value = false
  }
...
<template>
  <div class="p-4 sm:p-6 space-y-6">
    ...
    <!-- Loading skeleton -->
    <div v-if="loading" aria-busy="true" aria-label="Carregando tipos de atendimento" class="space-y-2">
    ...
    >
      ...
      <h2 class="text-base font-semibold text-foreground mb-1">Nenhum tipo de atendimento cadastrado</h2>
      ...
      <div class="hidden md:block overflow-x-auto rounded-lg border border-border">
        <table class="w-full text-sm">
          <thead class="bg-surface-muted sticky top-0">
            <tr>
              <th class="px-4 py-3 text-left font-medium text-foreground-muted">Nome</th>
              <th class="px-4 py-3 text-right font-medium text-foreground-muted">Duração</th>
              <th class="px-4 py-3 text-right font-medium text-foreground-muted">Valor particular</th>
              <th class="px-4 py-3 text-center font-medium text-foreground-muted">Situação</th>
              <th class="px-4 py-3 text-right font-medium text-foreground-muted sr-only">Ações</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-border">
            ...
            >
              <td class="px-4 py-3">
                ...
                <p v-if="t.descricao" class="mt-0.5 text-xs text-foreground-muted line-clamp-1">{{ t.descricao }}</p>
              </td>
              <td class="px-4 py-3 text-right tabular-nums text-foreground">
                {{ t.duration_minutes }} min
                <span v-if="t.buffer_minutes" class="text-foreground-muted text-xs"> +{{ t.buffer_minutes }} buf</span>
              </td>
              <td class="px-4 py-3 text-right tabular-nums text-foreground">
        ...
        >
          <div class="flex items-start justify-between gap-2">
            <div class="flex items-center gap-2 flex-1 min-w-0">
              ...
              <div class="min-w-0">
                ...
                <p v-if="t.descricao" class="text-xs text-foreground-muted truncate">{{ t.descricao }}</p>
              </div>
            </div>
            <span
          ...
          <dl class="grid grid-cols-2 gap-2 text-sm">
            <div>
              ...
              <dd class="font-medium text-foreground">{{ t.duration_minutes }} min</dd>
            </div>
            <div>
              <dt class="text-xs text-foreground-muted">Valor particular</dt>
              <dd class="font-medium text-foreground">{{ formatCurrency(t.valor_particular) }}</dd>
            </div>
          </dl>
          ...
          <div class="flex items-center gap-3 pt-1 border-t border-border">
            ...
            <button
              v-if="t.is_active"
              type="button"
              class="flex-1 rounded-lg border border-danger-500/50 px-3 py-2 text-sm font-medium text-danger-500 hover:bg-danger-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-danger-500 transition"
              @click="openInactivate(t)"
            >
              Inativar
            </button>
      ...
      >
        ...
        >
          <div class="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 id="type-form-modal-title" class="text-base font-semibold text-foreground">
              {{ editing?.id ? 'Editar tipo de atendimento' : 'Novo tipo de atendimento' }}
            </h2>
            <button
              type="button"
      ...
      >
        ...
        >
          <div class="px-6 py-4 border-b border-border">
            <h2 id="inactivate-modal-title" class="text-base font-semibold text-foreground">
              Inativar tipo de atendimento
            </h2>
          </div>
          <div class="px-6 py-4">
            <p class="text-sm text-foreground">
              Inativar <strong class="font-semibold">"{{ inactivateTarget?.nome }}"</strong>?
            </p>
            <p class="mt-2 text-xs text-foreground-muted">
              O histórico de consultas é preservado (FR-007). O tipo não estará disponível para novos agendamentos.
            </p>
          </div>
