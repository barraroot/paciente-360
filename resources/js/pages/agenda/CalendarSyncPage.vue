...
async function connect() {
  ...
  try {
    ...
    showToast(
      e?.response?.data?.message ?? 'Não foi possível iniciar a conexão com o Google. Tente novamente.',
      'error'
    )
  } finally {
...
<template>
  <div class="p-4 sm:p-6 space-y-6 max-w-2xl">
    ...
    <!-- ─── Google Calendar ──────────────────────────────────────────────────── -->
    <section class="rounded-lg border border-border bg-surface-elevated shadow-card overflow-hidden">
      <!-- Header da seção -->
      <div class="flex items-center gap-3 border-b border-border px-5 py-4">
        <!-- Ícone Google Calendar (SVG simplificado) -->
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0" viewBox="0 0 24 24" aria-hidden="true">
          <rect x="3" y="3" width="18" height="18" rx="2" fill="#4285F4" />
          <rect x="3" y="9" width="18" height="2" fill="#fff" opacity="0.3" />
          <text x="12" y="17" text-anchor="middle" font-size="7" fill="#fff" font-family="sans-serif" font-weight="bold">Cal</text>
        </svg>
        <div>
      ...
      <!-- Loading skeleton -->
      <div v-if="loading" aria-busy="true" aria-label="Verificando status da sincronização" class="px-5 py-6 space-y-3">
      ...
      <!-- Error state -->
      <div v-else-if="error" class="px-5 py-6 flex flex-col sm:flex-row sm:items-center gap-3">
      ...
      <template v-else-if="account && account.status === 'connected'">
        <div class="px-5 py-5 space-y-4">
          <div class="flex items-start gap-3">
            <span class="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-success-50">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-success-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
              </svg>
            </span>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-semibold text-foreground">Conectado</p>
              <p class="text-sm text-foreground-muted truncate">{{ account.provider_email }}</p>
            </div>
          </div>
          ...
          <dl class="space-y-2 text-sm">
            <div class="flex items-center justify-between gap-2">
              ...
              <dd class="font-mono text-xs text-foreground truncate max-w-[200px]">{{ account.google_calendar_id || '(padrão)' }}</dd>
            </div>
            <div v-if="account.last_synced_at" class="flex items-center justify-between gap-2">
            ...
            <div v-if="watchStatus" class="flex items-center justify-between gap-2">
              ...
              <dd
                aria-live="polite"
                :class="watchStatus.includes('expir') && !watchStatus.includes('dias') ? 'text-warning-500' : 'text-foreground'"
              >
                {{ watchStatus }}
              </dd>
            </div>
        ...
        <div class="flex justify-end border-t border-border px-5 py-3">
          <button
            type="button"
            class="text-sm text-danger-500 hover:text-danger-600 focus-visible:outline focus-visible:outline-2 focus-visible:outline-danger-500 rounded transition"
      ...
      <template v-else-if="account && account.status === 'disconnected'">
        <div class="px-5 py-5 space-y-3">
          <div class="flex items-start gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0 text-warning-500 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
            </svg>
            <div>
              <p class="text-sm font-semibold text-foreground">Sincronização interrompida</p>
              <p class="text-sm text-foreground-muted mt-0.5">{{ humanizeError(account) }}</p>
            </div>
          </div>

          <!-- Seletor de profissional -->
          <div v-if="professionals.length > 0">
            <label for="csp-prof-reconnect" class="block text-sm font-medium text-foreground mb-1">Profissional</label>
            <select
      ...
      <template v-else>
        <div class="px-5 py-6 space-y-4">
          <div>
            ...
            <ul class="space-y-2 text-sm text-foreground-muted">
              <li class="flex items-start gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0 text-primary-700 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                Consultas criadas no Paciente 360 aparecem automaticamente no Google Calendar do profissional
              </li>
              <li class="flex items-start gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0 text-primary-700 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              ...
              <li class="flex items-start gap-2">
                ...
                O profissional recebe notificações nativas do Google (app mobile, email)
              </li>
            </ul>
          </div>
          ...
          <!-- Seletor de profissional -->
          <div v-if="professionals.length > 0">
            <label for="csp-prof-connect" class="block text-sm font-medium text-foreground mb-1">
          ...
          <p v-else class="text-xs text-foreground-muted italic">
            Nenhum profissional cadastrado. Adicione profissionais antes de conectar o calendário.
          </p>
    ...
    <section class="rounded-lg border border-border bg-surface overflow-hidden" aria-label="Microsoft Outlook — indisponível">
      <div class="flex items-center gap-3 border-b border-border px-5 py-4 opacity-60">
        <!-- Ícone Outlook simplificado -->
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0" viewBox="0 0 24 24" aria-hidden="true">
          <rect x="3" y="3" width="18" height="18" rx="2" fill="#0078d4" />
          <text x="12" y="16" text-anchor="middle" font-size="9" fill="#fff" font-family="sans-serif" font-weight="bold">Ol</text>
        </svg>
        <div>
          <div class="flex items-center gap-2">
            <h2 class="text-base font-semibold text-foreground-muted">Microsoft Outlook</h2>
            <span class="inline-flex items-center rounded-full bg-surface-muted px-2 py-0.5 text-xs font-medium text-foreground-muted border border-border">
              Em breve
            </span>
          </div>
      ...
      <div class="px-5 py-5 opacity-60">
        <p class="text-sm text-foreground-muted">
          A integração com Microsoft Outlook / Microsoft 365 Calendar estará disponível em uma versão futura.
          A sincronização com Google Calendar já está disponível acima.
        </p>
        <button
          type="button"
          disabled
      ...
      >
        ...
        >
          <div class="px-6 py-4 border-b border-border">
            <h2 id="disconnect-modal-title" class="text-base font-semibold text-foreground">
              Desconectar Google Calendar
            </h2>
          </div>
          <div class="px-6 py-4 space-y-2">
            <p class="text-sm text-foreground">
              Deseja desconectar a conta <strong class="font-semibold">{{ account?.provider_email }}</strong>?
            </p>
            <p class="text-sm text-foreground-muted">
              Suas consultas existentes no Google Calendar <strong>permanecem</strong> — elas não serão apagadas.
              Novas criações, cancelamentos e reagendamentos no Paciente 360 <strong>não serão espelhados</strong> até reconectar.
            </p>
          </div>
