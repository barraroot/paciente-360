...
const REASON_LABELS = {
    ...
    prescription_outside_renewal_window:
        'Receita fora da janela de renovação (mais de 30 dias do vencimento).',
};
...
<template>
    <div class="min-h-screen bg-background">
        <div class="mx-auto max-w-2xl px-4 py-6 sm:px-6">
            ...
            <nav
                class="mb-1 flex items-center gap-1 text-xs text-foreground-muted"
            ...
            <!-- Loading skeleton -->
            <div
                v-if="store.loading && !original"
                aria-busy="true"
                aria-label="Carregando receita original"
                class="space-y-4"
            >
            ...
            <template v-else-if="original || !store.loading">
                <!-- Banner informativo -->
                <div
                    class="mb-6 flex items-start gap-3 rounded-xl border border-primary-200 bg-primary-50 px-4 py-3"
                    role="note"
                ...
                >
                    ...
                    <p class="text-sm text-primary-800">
                        Esta renovação criará uma nova receita vinculada à original. A receita
                        original será marcada como <strong>'superseded'</strong> automaticamente
                        após a criação.
                    </p>
                </div>
                ...
                <div
                    v-if="original"
                    class="mb-6 rounded-xl border border-border bg-surface p-4 flex flex-wrap items-center gap-2"
                >
                    ...
                    <span class="text-xs text-foreground-muted">
                        ...
                        {{
                            original.issued_at
                                ? DateTime.fromISO(original.issued_at).toFormat('dd/MM/yyyy')
                                : '—'
                        }}
                    </span>
                </div>
                ...
                <form class="space-y-6" novalidate @submit.prevent="submit">
                    ...
                    <div class="rounded-xl border border-border bg-surface p-4 space-y-2">
                        ...
                        <div
                            class="flex items-center gap-2 rounded-lg border border-border bg-surface-elevated px-3 py-2"
                        >
                            <svg
                                class="h-4 w-4 text-foreground-muted shrink-0"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke-width="1.5"
                                stroke="currentColor"
                    ...
                    <div class="rounded-xl border border-border bg-surface p-4 space-y-3">
                        ...
                        <div
                            ...
                            role="radiogroup"
                            aria-label="Duração da validade"
                        >
                        ...
                        <div
                            v-else
                            class="flex items-center gap-2 rounded-lg bg-warning-50 border border-warning-200 px-4 py-3"
                        >
                            <svg
                        ...
                        <p v-if="expiresAtDisplay" class="text-xs text-foreground-muted">
                            Nova receita expira em
                            <strong class="text-foreground">{{ expiresAtDisplay }}</strong>
                        </p>
                    </div>
                    ...
                    <div class="rounded-xl border border-border bg-surface p-4 space-y-2">
                        <label
                            for="renew-appointment"
                            class="block text-sm font-medium text-foreground"
                        >
                            Consulta vinculada
                            <span class="ml-1 text-xs font-normal text-foreground-muted"
                                >(opcional)</span
                            >
                        </label>
                        <input
