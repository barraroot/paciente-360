...
function subscribeReportRefresh() {
  ...
  try {
    echo.private(`prescriptions.${tenantId}`)
      .listen('.PrescriptionsReportRefresh', async () => {
...
<template>
  <div class="min-h-screen bg-background">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <!-- Breadcrumb -->
      <nav class="mb-2 flex items-center gap-1 text-xs text-foreground-muted" aria-label="Breadcrumb">
        <router-link
      ...
      <!-- Loading skeleton -->
      <div v-if="reportLoading && reportList.length === 0" aria-busy="true" aria-label="Carregando relatório" class="space-y-2">
        <div v-for="i in 6" :key="i" class="h-14 rounded-xl bg-surface-elevated animate-pulse" />
      </div>
      ...
      >
        ...
        <p class="text-sm text-foreground-muted">Nenhuma receita encontrada com os filtros selecionados.</p>
      </div>
          ...
          <table class="min-w-full divide-y divide-border text-sm">
            <thead class="bg-surface-elevated">
              <tr>
                ...
                <th scope="col" class="hidden px-4 py-3 text-left text-xs font-semibold text-foreground-muted uppercase tracking-wide lg:table-cell">Renovada</th>
              ...
              >
                ...
                <td class="px-4 py-3 font-medium text-foreground">
                  <span
                    ...
                    :title="$t ? $t('prescription.restricted_access') : 'Acesso restrito'"
                ...
                <td class="hidden px-4 py-3 text-foreground-muted sm:table-cell">
                  <span v-if="prescription.masked" class="tracking-widest" :title="$t ? $t('prescription.restricted_access') : 'Acesso restrito'">•••</span>
                  <span v-else>{{ prescription.professional_name ?? '—' }}</span>
                </td>

                <!-- Tipo -->
                <td class="px-4 py-3">
                  <span v-if="prescription.masked" class="tracking-widest text-foreground-muted" :title="$t ? $t('prescription.restricted_access') : 'Acesso restrito'">•••</span>
                ...
                <td class="px-4 py-3 text-right">
                  <div v-if="prescription.masked" class="text-xs text-foreground-muted italic">
                    Restrito
                  </div>
                  <div v-else class="flex items-center justify-end gap-2">
                    ...
                    <router-link
                      v-if="prescription.status === 'active' && !prescription.is_renewed"
                      :to="{ name: 'prescriptions.renew', params: { id: prescription.id } }"
      ...
      >
        ...
        >
          ...
          <div class="px-6 py-4 space-y-4">
            <!-- Aviso CONFIDENCIAL -->
            <div class="flex items-start gap-3 rounded-lg bg-warning-50 border border-warning-200 px-4 py-3">
              <svg class="mt-0.5 h-4 w-4 shrink-0 text-warning-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
              </svg>
              <p class="text-sm text-warning-800">
                Este export <strong>pode conter receitas controladas</strong>. O arquivo terá o prefixo
                <code class="rounded bg-warning-100 px-1 text-xs font-mono">CONFIDENCIAL_</code>
