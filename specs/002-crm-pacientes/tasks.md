...
- [x] T001 [P] Adicionar `league/csv` ao composer.json — `composer.json`
...
- [x] T002 [P] Adicionar `phpoffice/phpspreadsheet` ao composer.json — `composer.json`
...
- [x] T003 Configurar fila dedicada `imports` no Horizon — `config/horizon.php`, `config/queue.php`
...
- [x] T004 [P] Configuração de timeline tracked fields — `config/paciente.php`
...
- [x] T005 [P] Configuração de limites de importação — `config/paciente.php`
...
- [x] T006 [P] Configuração de rate limiters novos — `app/Providers/RouteServiceProvider.php`
...
- [x] T007 [P] Storage para arquivos de import — `config/filesystems.php`
...
- [x] T008 [P] Lang pt-BR estendido para CRM — `lang/pt_BR/paciente.php`
...
- [x] T009 [P] Vue i18n pt-BR estendido — `resources/js/i18n/pt-BR.json`
...
- [x] T009a [P] Adicionar `vuedraggable@^4` ao `package.json` — `package.json`
...
- [x] T010 Migration `enable_pg_trgm_and_unaccent` — `database/migrations/2026_05_11_000001_enable_pg_trgm_and_unaccent.php`
...
- [x] T011 [P] Migration `pacientes` — `database/migrations/2026_05_11_000002_create_pacientes_table.php`
...
- [x] T012 [P] Migration `convenios` — `database/migrations/2026_05_11_000003_create_convenios_table.php`
  - Descrição: schema §2; UNIQUE `(tenant_id, nome)`.
  - Aceitação: tabela criada com índice unique.
  - Depende de: T010
  - Princípio: II

- [x] T013 [P] Migration `paciente_convenios` — `database/migrations/2026_05_11_000004_create_paciente_convenios_table.php`
...
- [x] T014 [P] Migration `tags` — `database/migrations/2026_05_11_000005_create_tags_table.php`
...
- [x] T015 [P] Migration `paciente_tags` — `database/migrations/2026_05_11_000006_create_paciente_tags_table.php`
  - Descrição: schema §5; UNIQUE `(paciente_id, tag_id)`.
  - Aceitação: tag não duplica em paciente.
  - Depende de: T011, T014
  - Princípio: II

- [x] T016 Migration `anotacoes` + trigger imutabilidade — `database/migrations/2026_05_11_000007_create_anotacoes_table.php`
...
- [x] T017 Migration `eventos_timeline` + trigger imutabilidade — `database/migrations/2026_05_11_000008_create_eventos_timeline_table.php`
...
- [x] T018 [P] Migration `importacoes` — `database/migrations/2026_05_11_000009_create_importacoes_table.php`
...
- [x] T019 [P] Migration `mesclagens_pacientes` — `database/migrations/2026_05_11_000010_create_mesclagens_pacientes_table.php`
  - Descrição: schema §9. Index `(tenant_id, reversivel_ate, revertida_em)` para purge mensal.
  - Aceitação: tabela criada.
  - Depende de: T011
  - Princípio: I, V

- [x] T020 [P] Migration `funil_colunas` — `database/migrations/2026_05_11_000011_create_funil_colunas_table.php`
  - Descrição: schema §10. UNIQUE `(tenant_id, slug)` e `(tenant_id, posicao)`.
  - Aceitação: insert ok; colisão de slug falha.
  - Depende de: T010
  - Princípio: II

- [x] T021 [P] Migration `tarefas_reatribuicao` — `database/migrations/2026_05_11_000012_create_tarefas_reatribuicao_table.php`
  - Descrição: schema §11. Index `(tenant_id, concluida_em)` para listar pendentes.
  - Aceitação: tabela criada.
  - Depende de: T010
  - Princípio: II

- [x] T022 Migration índices trigram em `pacientes` — `database/migrations/2026_05_11_000013_add_pacientes_trigram_indexes.php`
...
- [x] T023 [P] Model `Paciente` com trait `BelongsToTenant` + casts + relações — `app/Models/Paciente.php`
...
- [x] T024 [P] Models `Convenio` + `PacienteConvenio` — `app/Models/Convenio.php`, `app/Models/PacienteConvenio.php`
...
- [x] T025 [P] Models `Tag` + `PacienteTag` — `app/Models/Tag.php`, `app/Models/PacienteTag.php`
...
- [x] T026 [P] Model `Anotacao` + bloqueio em `updating`/`deleting` — `app/Models/Anotacao.php`
...
- [x] T027 [P] Model `EventoTimeline` + bloqueio imutabilidade — `app/Models/EventoTimeline.php`
  - Descrição: mesmo padrão de `AuditLog` (Fase 0). Casts `payload=>AsJsonArray`.
  - Aceitação: update/delete via Model lança exceção.
  - Depende de: T017
  - Princípio: I, V

- [x] T028 [P] Models `Importacao`, `MesclagemPaciente`, `FunilColuna`, `TarefaReatribuicao` — `app/Models/*.php`
...
- [x] T029 [P] Exception `AnotacaoImutavelException` — `app/Exceptions/Pacientes/AnotacaoImutavelException.php`
...
- [x] T030 [P] **TEST** Permissões CRM aplicadas por perfil — `tests/Feature/Fase2/Pacientes/PacientePermissionsTest.php`
...
- [x] T031 Seeder Spatie estendido com abilities CRM — `database/seeders/RolesSeeder.php`
...
- [x] T032 Policies — `app/Policies/{PacientePolicy,AnotacaoPolicy,TagPolicy,ConvenioPolicy,FunilPolicy}.php`
...
- [x] T033 [P] Estender `AuditAttributesBuilder` para mascarar CPF — `app/Services/Audit/AuditAttributesBuilder.php`
...
- [x] T034 [P] 13 Events da Fase 2 (skeleton) — `app/Events/Paciente/*.php`
...
- [x] T035 [P] Listener `RegistraEventoTimelineListener` — `app/Listeners/Paciente/RegistraEventoTimelineListener.php`
...
- [x] T036 [P] `CpfValidator` (DV algoritmo BR) — `app/Support/Cpf/CpfValidator.php` + teste unit
...
- [x] T037 [P] `TelefoneNormalizer` (E.164 + display BR) — `app/Support/Telefone/TelefoneNormalizer.php` + teste unit
...
- [x] T038 [P] `TagNormalizer` (case + accent-insensitive) — `app/Support/Tags/TagNormalizer.php` + teste unit
...
- [x] T039 [P] Estender `DevSeeder` com 30 pacientes em `clinica-alfa` — `database/seeders/DevSeeder.php`
...
- [x] T100 [P] [US1] **TEST** Cadastro happy path — `tests/Feature/Fase2/Pacientes/PacienteCadastroTest.php`
...
- [x] T101 [P] [US1] **TEST** Validação de CPF — `tests/Feature/Fase2/Pacientes/PacienteCpfValidationTest.php`
...
- [x] T102 [P] [US1] **TEST** Deduplicação na criação — `tests/Feature/Fase2/Pacientes/PacienteDedupTest.php`
...
- [x] T103 [P] [US1] **TEST** Isolamento entre tenants — `tests/Feature/Fase2/Pacientes/PacienteIsolationTest.php`
...
- [x] T104 [P] [US1] **TEST** Mesclagem reversível — `tests/Feature/Fase2/Pacientes/PacienteMergeTest.php`
...
- [x] T105 [P] [US1] **TEST** Anonimização (stub LGPD) — `tests/Feature/Fase2/Pacientes/PacienteAnonimizacaoTest.php`
...
- [x] T106 [P] [US1] **TEST** Busca por similaridade — `tests/Feature/Fase2/Pacientes/PacienteSearchTest.php`
...
- [x] T110 [US1] `PacienteService` (cria/atualiza/anonimiza) — `app/Services/Pacientes/PacienteService.php`
...
- [x] T111 [US1] `DedupService` — `app/Services/Pacientes/DedupService.php`
...
- [x] T112 [US1] `MergeService` (mesclagem reversível) — `app/Services/Pacientes/MergeService.php`
...
- [x] T113 [US1] `AnonimizacaoService` — `app/Services/Pacientes/AnonimizacaoService.php`
...
- [x] T114 [US1] Form Requests — `app/Http/Requests/Pacientes/{Create,Update}PacienteRequest.php`
...
- [x] T115 [US1] Form Request `MesclagemRequest` — `app/Http/Requests/Pacientes/MesclagemRequest.php`
...
- [x] T116 [US1] Resources — `app/Http/Resources/{Paciente,PacienteList,Convenio,PacienteConvenio,Mesclagem}Resource.php`
...
- [x] T117 [US1] `PacientesController` (CRUD + busca + anonimizar) — `app/Http/Controllers/Api/V1/Pacientes/PacientesController.php`
...
- [x] T118 [US1] `MesclagemController` — `app/Http/Controllers/Api/V1/Pacientes/MesclagemController.php`
...
- [x] T119 [US1] `PatchStatusController` (transição de status) — `app/Http/Controllers/Api/V1/Pacientes/PatchStatusController.php`
...
- [x] T120 [US1] Rotas em `routes/api.php`
...
- [x] T121 [US1] `PacienteFactory` — `database/factories/PacienteFactory.php`
...
- [x] T122 [US1] `ConvenioFactory` — `database/factories/ConvenioFactory.php`
...
- [x] T123 [P] [US1] Vue: `PacientesListPage.vue` — `resources/js/pages/pacientes/PacientesListPage.vue`
...
- [x] T124 [P] [US1] Vue: `PacienteFormPage.vue` (criar/editar) — `resources/js/pages/pacientes/PacienteFormPage.vue`
...
- [x] T125 [P] [US1] Vue: `PacienteShowPage.vue` (ficha) — `resources/js/pages/pacientes/PacienteShowPage.vue`
...
- [x] T126 [P] [US1] Vue: `DedupSuggestionModal.vue` — `resources/js/components/pacientes/DedupSuggestionModal.vue`
...
- [x] T127 [P] [US1] Vue: `MesclagemPage.vue` — `resources/js/pages/pacientes/MesclagemPage.vue`
...
- [x] T128 [P] [US1] Composable `usePacienteSearch` — `resources/js/composables/usePacienteSearch.js`
...
- [x] T129 [US1] Router + i18n keys US1 — `resources/js/router/index.js`, `resources/js/i18n/pt-BR.json`
...
- [x] T140 [P] [US2] **TEST** Timeline com eventos próprios — `tests/Feature/Fase2/Pacientes/PacienteTimelineTest.php`
...
- [x] T141 [P] [US2] **TEST** Anotações tipadas e visibilidade — `tests/Feature/Fase2/Pacientes/PacienteAnotacaoTest.php`
...
- [x] T142 [P] [US2] **TEST** Imutabilidade + retratação — `tests/Feature/Fase2/Pacientes/AnotacaoImutabilidadeTest.php`
...
- [x] T150 [US2] `TimelineService` (query da timeline) — `app/Services/Pacientes/TimelineService.php`
...
- [x] T151 [US2] `AnotacaoService` — `app/Services/Pacientes/AnotacaoService.php`
...
- [x] T152 [US2] Form Requests — `app/Http/Requests/Pacientes/{CreateAnotacao,Retratacao}Request.php`
...
- [x] T153 [US2] Resources — `app/Http/Resources/{Anotacao,EventoTimeline,Timeline}Resource.php`
...
- [x] T154 [US2] `TimelineController` + `AnotacoesController` — `app/Http/Controllers/Api/V1/Pacientes/{TimelineController,AnotacoesController}.php`
...
- [x] T155 [US2] Rotas timeline + anotações — `routes/api.php`
...
- [x] T156 [P] [US2] `AnotacaoFactory` — `database/factories/AnotacaoFactory.php`
...
- [x] T157 [P] [US2] Vue: `PacienteTimelineTab.vue` — `resources/js/components/pacientes/PacienteTimelineTab.vue`
...
- [x] T158 [P] [US2] Vue: `TimelineEvent.vue` — `resources/js/components/pacientes/TimelineEvent.vue`
...
- [x] T159 [P] [US2] Vue: `AnotacaoForm.vue` + integração com timeline — `resources/js/components/pacientes/AnotacaoForm.vue`
...
- [x] T170 [P] [US3] **TEST** Fluxo completo de importação — `tests/Feature/Fase2/Pacientes/ImportacaoTest.php`
...
- [x] T171 [P] [US3] **TEST** Limites e formatos — `tests/Feature/Fase2/Pacientes/ImportacaoLimitsTest.php`
  - Descrição: cobre AC-3.3.3 (acima do limite). Arquivo > 5 MB → 413. > 10.000 linhas → 422. Cabeçalhos faltantes → 422.
  - Aceitação: 4 cenários verde.
  - Depende de: T005
  - Princípio: VII

- [x] T172 [P] [US3] **TEST** Reimportação atualiza apenas vazios — `tests/Feature/Fase2/Pacientes/ImportacaoReimportTest.php`
...
- [x] T173 [P] [US3] **TEST** Permissão restritiva (Admin apenas) — `tests/Feature/Fase2/Pacientes/ImportacaoPermissionTest.php`
  - Descrição: cobre AC-3.3.9. Médico/Atendente/Recepcionista tentam upload → 403. Admin Clínica → 202.
  - Aceitação: 4 cenários verde.
  - Depende de: T031
  - Princípio: VII

- [x] T174 [P] [US3] **TEST** Retomada após falha de worker — `tests/Feature/Fase2/Pacientes/ImportacaoRetomadaTest.php`
...
- [x] T180 [US3] `ImportacaoService` (orquestrador) — `app/Services/Pacientes/ImportacaoService.php`
...
- [x] T181 [US3] `ProcessPatientImportJob` extends `TenantAwareJob` — `app/Jobs/Pacientes/ProcessPatientImportJob.php`
...
- [x] T182 [US3] `CsvImportParser` + `XlsxImportParser` — `app/Services/Pacientes/Importadores/{CsvImportParser,XlsxImportParser}.php`
...
- [x] T183 [US3] `LinhaImportacaoValidator` — `app/Services/Pacientes/Importadores/LinhaImportacaoValidator.php`
...
- [x] T184 [US3] Template generator — `app/Services/Pacientes/TemplateGenerator.php`
...
- [x] T185 [US3] Form Request `ImportPacientesRequest` — `app/Http/Requests/Pacientes/ImportPacientesRequest.php`
...
- [x] T186 [US3] Resources `ImportacaoResource` — `app/Http/Resources/ImportacaoResource.php`
...
- [x] T187 [US3] `ImportacaoController` (template + upload + status) — `app/Http/Controllers/Api/V1/Pacientes/ImportacaoController.php`
...
- [x] T188 [US3] Rotas import — `routes/api.php`
...
- [x] T189 [P] [US3] Vue: `ImportacaoPage.vue` (upload) — `resources/js/pages/pacientes/ImportacaoPage.vue`
...
- [x] T190 [P] [US3] Vue: `ImportacaoStatusPage.vue` (progresso + relatório) — `resources/js/pages/pacientes/ImportacaoStatusPage.vue`
...
- [x] T191 [P] [US3] Vue: rotas e i18n US3 — `resources/js/router/index.js`, `resources/js/i18n/pt-BR.json`
  - Descrição: 2 rotas (`import.upload`, `import.status`); chaves i18n `import.*`.
  - Aceitação: navegação ok.
  - Depende de: T189, T190
  - Princípio: Localização

- [x] T192 [P] [US3] `ExportacaoService` (cross-cutting com US1) — `app/Services/Pacientes/ExportacaoService.php`
...
- [x] T193 [US3] `ExportacaoController` — `app/Http/Controllers/Api/V1/Pacientes/ExportacaoController.php`
...
- [x] T194 [US3] Rota export — `routes/api.php`
...
- [x] T200 [P] [US4] **TEST** Funil Kanban — `tests/Feature/Fase2/Pacientes/FunilKanbanTest.php`
...
- [x] T201 [P] [US4] **TEST** Configuração de colunas — `tests/Feature/Fase2/Pacientes/FunilConfigTest.php`
...
- [x] T202 [P] [US4] **TEST** Movimentação automática (gancho Fase 5) — `tests/Feature/Fase2/Pacientes/FunilAutoMoveTest.php`
...
- [x] T210 [US4] `FunilTemplateService` (lazy init) — `app/Services/Funil/FunilTemplateService.php`
...
- [x] T211 [US4] `FunilService` (movimentação) — `app/Services/Funil/FunilService.php`
...
- [x] T212 [US4] Form Request `MoveCardRequest` — `app/Http/Requests/Pacientes/MoveCardRequest.php`
...
- [x] T213 [US4] Resources `FunilColunaResource` — `app/Http/Resources/FunilColunaResource.php`
  - Descrição: serialização conforme contrato; inclui `pacientes_count` em listagens.
  - Aceitação: shape bate.
  - Depende de: T210
  - Princípio: Restrições Técnicas

- [x] T214 [US4] `FunilController` + `FunilColunasController` — `app/Http/Controllers/Api/V1/Pacientes/FunilController.php`
...
- [x] T215 [US4] Rotas funil — `routes/api.php`
  - Descrição: `GET/PATCH funil/colunas/{id?}`, `PATCH pacientes/{id}/funil`.
  - Aceitação: 3 rotas em route:list.
  - Depende de: T214
  - Princípio: II

- [x] T216 [P] [US4] Vue: `FunilKanbanPage.vue` (board) — `resources/js/pages/pacientes/FunilKanbanPage.vue`
...
- [x] T217 [P] [US4] Vue: `KanbanBoard.vue` + `KanbanColumn.vue` + `PacienteCard.vue` — `resources/js/components/funil/*.vue`
...
- [x] T218 [P] [US4] Vue: `FunilConfigPage.vue` (configurar colunas) — `resources/js/pages/pacientes/FunilConfigPage.vue`
  - Descrição: form simples para Admin renomear/reordenar/marcar `motivo_obrigatorio` em colunas. Drag-and-drop para reordenar.
  - Aceitação: smoke test estático.
  - Depende de: T215
  - Princípio: Restrições Técnicas

- [x] T219 [P] [US4] Vue: `PerdidoMotivoModal.vue` — `resources/js/components/pacientes/PerdidoMotivoModal.vue`
...
- [x] T230 [P] [US5] **TEST** Tags globais + normalização — `tests/Feature/Fase2/Pacientes/TagSegmentacaoTest.php`
...
- [x] T231 [P] [US5] **TEST** Máquina de estados — `tests/Feature/Fase2/Pacientes/PacienteStatusMachineTest.php`
...
- [x] T232 [P] [US5] **TEST** Convênios CRUD + uso — `tests/Feature/Fase2/Pacientes/ConvenioCrudTest.php`
...
- [x] T240 [US5] `TagService` (find or create + normalização) — `app/Services/Pacientes/TagService.php`
...
- [x] T241 [US5] `StatusMachineService` — `app/Services/Pacientes/StatusMachineService.php`
...
- [x] T242 [US5] `ConvenioService` — `app/Services/Convenios/ConvenioService.php`
...
- [x] T243 [US5] Form Requests — `app/Http/Requests/{Pacientes/CreateTagRequest,Convenios/CreateConvenioRequest}.php`
...
- [x] T244 [US5] Resources `TagResource` + `ConvenioResource` — `app/Http/Resources/{Tag,Convenio}Resource.php`
  - Descrição: serialização. `TagResource` inclui `pacientes_count` opcional.
  - Aceitação: shape bate com contrato.
  - Depende de: T240, T242
  - Princípio: Restrições Técnicas

- [x] T245 [US5] `TagsController` + `ConveniosController` + `PacienteTagsController` — `app/Http/Controllers/Api/V1/Pacientes/*.php`
...
- [x] T246 [US5] Rotas tags + convenios — `routes/api.php`
...
- [x] T247 [P] [US5] `TagFactory` — `database/factories/TagFactory.php`
  - Descrição: factory para tags. State `sistemica()` aplica prefixo `sys:`.
  - Aceitação: usada em testes.
  - Depende de: T025
  - Princípio: IV

- [x] T248 [P] [US5] Vue: `TagPicker.vue` + ConvenioForm + ConveniosListPage — `resources/js/components/pacientes/TagPicker.vue`, `resources/js/pages/convenios/{ConveniosListPage,ConvenioFormPage}.vue`
...
- [x] T249 [P] [US5] Vue: extender `PacienteShowPage.vue` com seções status/tags/convenios — `resources/js/pages/pacientes/PacienteShowPage.vue`
