...
| **VI. Conformidade Meta (NON-NEGOTIABLE)** | PASS c/ ressalva | Fora da janela 24h → `OutboundMessage` com `contentType='template'` (bypass de janela já implementado em `enforceWindow24h`). **Ressalva**: a verificação de *aprovação* do template é delegada à configuração do tenant (só configura templates aprovados) + rejeição do provedor → status `failed` → fallback manual. Ver research §6 para a estratégia de verificação. Notificações desta feature são **transacionais** (vinculadas a uma consulta/receituário do próprio paciente) — não disparo de marketing em massa, logo as cláusulas de opt-in de marketing/“/sair” do Princípio VI se aplicam apenas como reuso do opt-out já existente, não como novo gate de massa. |
...
**Resultado**: PASS 7/7 — nenhuma violação que exija Complexity Tracking. A única ressalva (verificação de aprovação de template) é tratada em research §6 com estratégia de defesa em profundidade, sem amendment constitucional.
